...
class DoubleMainAgent(BaseAgent):
    ...
    def process_result(self, state: ClassifierAgentState) -> CitedAIMessage:
        difficulty = state["classifier_result"]
        if difficulty.difficulty == "HARD":
            result = self.complex_main_agent.process_result(state.get("main_final_state"))
        else:
            result = self.simple_main_agent.process_result(state.get("main_final_state"))

        print(f"Pregunta es {difficulty.difficulty}")
        return result

    async def execute_main_agent(self, state: ClassifierAgentState) -> ClassifierAgentState:
        ...
        state["main_final_state"] = main_final_state
        return state
    ...
    def create_graph(self) -> CompiledGraph:
        ...
        classifier_graph = self.classifier_agent.create_graph()

        graph_builder.add_node("classifier_agent", classifier_graph)
